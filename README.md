...
The project only uses a two specialized `assign` and `index` special forms (macros) to satisfy Lua assignment and indexing semantics in Lisp.  The rest of the output is constructed from plain and simple Lisp with `let`, `letrec`, `block`, `return-from`, `begin` (or `progn`), `cond`, `if`, `lambda`, `define`, `list`, `cons`, and arithmetic/relational operators.
